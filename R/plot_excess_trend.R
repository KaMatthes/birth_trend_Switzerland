...
# }
    plot_birth <- ggplot()+
      ...
      scale_x_date(labels = date_format("%Y"), 
                   ...
                   limits =c(min(ymd("2018-01-01")), max(ymd("2022-12-01")))) +# coord_cartesian(ylim=c(0, 50)) +
      ggtitle(Title) +
      # xlim(1910, 1968) +
      xlab("Year") +
      # ylab("Births per 10'000 inhabitants")+
    ...
    plot_excess <- ggplot() +
      ...
      scale_x_date(labels = date_format("%Y"), 
                   breaks = date_breaks("1 year"),
                   # limits =c(min(ymd("2018-01-01")), max(ymd("2023-09-01")))) +
      limits =c(min(ymd("2018-01-01")), max(ymd("2022-12-01")))) +
      scale_y_continuous(labels = scales::percent) +
      scale_fill_manual("",
                        breaks=c("non-significant","significant"),
                        values =c("grey","red")) +
  ...
  cowplot::save_plot(paste0("output/plot_birth_",group_data,".pdf"),   plot_together ,base_height=12,base_width=15)
  
}

function_plot_expected(group_data="parity_1",Title="Parity = 1")
function_plot_expected(group_data="parity_sup_1",Title="Parity > 1")
...
function_plot_expected(group_data="total_birth",Title="Total population")
# function_plot_expected(group_data="total_birth_females",pop_group="pop",Title="Population women aged 15-49")

function_plot_expected(group_data="mat_age_below_30",Title="Birth age < 30")
function_plot_expected(group_data="mat_age_above_or_eq_30",Title=" Birth >=30")
